...
8. `asteroid-profile.js` → `window.AsteroidProfile` (unified per-player save: name, character, claim, journal, inventory, missions, crafting recipes, planet catalog)
9. `asteroid-creatures.js` → `window.AsteroidCreatures` (bestiary content registry — 12 creature Actors + fx kit)
10. `asteroid-ships.js` → `window.AsteroidShips` (voxel fleet registry — 23 ship Actors + InstancedMesh renderer)
11. `voxelworld.js` → `window.VoxelWorld` (Asteroid voxel sandbox; consumes the two registries above, optional via `window.*` guards)
12. `modes.js` → `window.ModeRegistry` and `window.PJBOY_MODE_DEFS`
13. `game.js` → defines `AudioBus` and the `Game3D` class; instantiates `new Game3D()` on `DOMContentLoaded`
14. `game-modes-impl.js` → **patches `Game3D.prototype` after the class is defined** (see below)
...
- **`voxelworld.js`** — `VoxelWorld` class wrapping a `createRuntime(game)` closure (~8.6k lines). Takes over `game.scene` / `game.camera` on `enter()`, restores on `exit()`. Own voxel grid, meshing, physics, HUD (`#voxel-overlay`), hotbar, block drawer, combat VFX, and camera (OTS third-person + first-person viewmodel).
  - **World grid:** height **`H=96`**, chunked at `CH=16`; horizontally it streams effectively endlessly — terrain wraps every `WORLD_PERIOD=3072` blocks (`W`/`D` alias `WORLD_PERIOD`) and chunks load/unload around the player (`streamAround`/`streamInit`/`resetStreaming`). `WORLD_OFFSET` maps voxel→render space (`y - SEA_LEVEL`); `SPACE_Y0=72`/`SPACE_Y1=148` are the atmosphere→space band. Deterministic from `SEED` (per-planet); the mesher is per-COLUMN (`scene3.chunks` keyed `"cx,cz"`, `buildColumnMesh` emits ONE merged mesh per material per 16×H×16 column with `matrixAutoUpdate=false`, `rebuildChunkAt`) — chosen because per-16³-sub-chunk meshes put tens of thousands of Object3Ds in the scene at far view radii. Block edits are a flat `{x,y,z,id}` list through `setBlockEvent` / `persistBlockEdit`, saved per-planet in the profile. Perf plumbing in `tick()`: heavy/idempotent per-frame work (streaming, `processBuildQueue`, DOM HUD, aim visuals, `adaptQuality`) is gated to once per RENDERED frame via a rAF frame clock, because the fixed-step accumulator calls `tick()` multiple times per frame when slow; `adaptQuality` measures real frame ms from that clock (tick's own dt is the constant 1/60 fixed step — never use it to judge frame cost) and DISCOUNTS ms spent in `processBuildQueue` (deliberate travel catch-up must not shed VIEW_R — that unloads terrain ahead of a moving player). Streaming is location-aware: the stream center leads the player along velocity, the build queue is sorted travel-priority (ahead ≈ half cost, behind ≈ double), and the meshing budget ramps 4→28 ms/frame as time-to-edge (distance/speed) drops below ~12s. **Seamless-horizon invariant: fog does NOT track VIEW_R — `updateFogFrontier` eases `FOG_FAR` toward the BUILT frontier (`minPendingDistAll` − margin), so columns only ever mesh behind the opaque fog wall and nothing pops in on screen. At altitude, `updateSky` caps fog.far at the SLANT distance to that built edge (`0.98·√(camY² + FOG_FAR²)`), so climbing reveals more world but the disc rim always fades into haze. Never snap fog to VIEW_R outside the loading screen.**
...
- **`asteroid-profile.js`** — `window.AsteroidProfile`: the unified per-player save (name, character cfg, claim seed+edits, journal/missions, inventory, weapon/drone tiers). One JSON document per profile, cloud-shaped. **Multi-profile:** named save slots live at `pjboy.profile.v2.<id>` with an index `pjboy.profiles.v1` (`{active, list}`); `load`/`save` target the **active** slot, and the legacy single `pjboy.profile.v2` is auto-migrated into slot `p1` on first run. Slot helpers: `listProfiles`/`activeProfileId`/`createProfile`/`switchProfile`/`deleteProfile`/`renameProfile`. Pure helpers (`load`/`save`, `missionProgress`, `recordScan`/`recordPlace`/`recordCraft`, `craftAvailability`, `weaponTier`/`droneTier`, `upsertBlockEdit`, `claimSummary`); `syncLegacyKeys` bridges the older `pjboy.voxel*` keys for the active profile. The **Profiles** screen (create/switch/rename/delete) is in `game-modes-impl.js` (`_showProfiles`/`_renderProfiles`), reached from the Asteroid "Your claim" screen.
...
#### Universe layer (the "PJBoy universe" — voxelworld + two content registries)

Asteroid mode is being grown into a voxel space survival-builder per the universe brief. Two **data-driven content registries** feed it via the universal **Actor contract** — `build() → {group: THREE.Group, st:{}, anim(t, state, dt)}` (the AI/owner sets `state` ∈ `idle|move|alert`; the renderer only calls `anim`). Both are loaded before `voxelworld.js` and consumed through optional `window.*` guards (`_AC`, `_AS`), so the sandbox still runs if either is absent.

- **`asteroid-creatures.js`** — `window.AsteroidCreatures`: the **bestiary** as a registry. `DEFS` (12 creatures: 6 peaceful Living that auto-spawn + gated hostiles/bosses/NPC) carries `faction/biome/temp/threat/on[]/scale/spawn/hp` + combat stats + educational `sci` scanner cards. Exposes `build(id)`, a self-contained toon-outline + spark `fx` kit (`setScene`/`spark`/`stepFx`/`clearFx`). Wired into voxelworld's (re-enabled) **critter system**: `placeCritter` builds the Actor, `updateCritters` drives idle/move/alert + a hostile `updateHostile` FSM (wander→approach→attack, vertical-aware), with the rare prowler spawner `maybeSpawnHostile`. Danish creature names live in voxelworld's `BLOCK_DA`.
- **`asteroid-ships.js`** — `window.AsteroidShips`: the **voxel fleet** as a registry. `BUILDERS` (23 ships by faction) paint into a voxel `Map`, rendered by one `InstancedMesh` per material (`build(name, {scale, tilt})`) with throttle-reactive engine flames, nav-light strobes, and a faction palette. `PILOTABLE` is the Hero fleet with per-ship `scale/speed/turn/wing`. Ships use the same Actor contract.
- **Combat** (`voxelworld.js`): `player.health/invuln/knock`, `applyPlayerDamage` (knockback fed through `moveAxis`, i-frames, slow regen, forgiving respawn at `_spawnPos`), hearts HUD (`#voxel-hearts`) + `#voxel-hurt-flash`; `fireCombat` → `pickCombatCreature` (reach measured from the **player**, not camera) → `damageCreature`/`killCreature`. Kid-forgiving by design.
- **Gear & upgrades** (`voxelworld.js` + `asteroid-profile.js`): each weapon/tool has a **tier** Mk I/II/III (`inventory.weaponTier`) and the Drone has `inventory.droneTier`. Tiers multiply base power/range (`TIER_MULT`) via `weaponGameplayFor` (cached in `_weaponTierCache`/`_droneTierCache`, refreshed by `refreshTierCache` — never per-frame `localStorage`), so upgrading boosts damage, reach **and** mining speed. Upgrades are crafted in the **Refinery** "Upgrades" panel (`renderUpgradesSection` → `upgradeWeapon`/`upgradeDrone`, costs `UPGRADE_COST`/`DRONE_COST` in Metal/Crystal/Void Crystal/Circuit). Drone tiers add assists in `updateDrone`: Mk I auto-catalogs nearby creatures, Mk II mends you, Mk III zaps hostiles. (The F8/F9 sliders remain dev-only; gameplay stats come from tier × base.) **Not yet done:** weapons are still free-equip (no craft-to-unlock acquisition gating).
- **Flight** (`voxelworld.js`): board/land a Hero ship with **G**, **C** swaps hull (hot-swap mid-flight). Momentum-based arcade model (`updateFlight`): nose follows velocity, coordinated banking, afterburner (Space+W), wingtip vapor trails. Chase cam (`updateFlightCamera`) rigidly behind the tail, tilts with pitch/roll, speed-reactive FOV. While piloting, `player.pos` rides the ship and the on-foot tick is skipped.
- **Travel** (`voxelworld.js` + `asteroid-profile.js` planet catalog): two paths into the **6-world catalog** (`AsteroidProfile.PLANETS`, biome+seed each; unlock via `grantPlanet`/missions). (1) A ground **Star Gate** (Ancient ring prop near spawn) — walk up, **E** to step through; dormant worlds cost a crafted **Gate Key** (block 41, recipe in `CRAFT_RECIPES`). (2) Fly a ship up past the space line → a **star map** of game-sized planet bodies appears (`enterSpaceMode`/`updateSpace`); fly to one to travel. Both reuse `travelToPlanet` (loading → regenerate seed/biome → restream). One eased `_mapFade` crossfades the pinned ground globe out as the discrete planet bodies fade in (smooth orbit entry/exit).
...
- **Persistence:** primary save is the active profile slot `pjboy.profile.v2.<id>` (index at `pjboy.profiles.v1`; see asteroid-profile above). Legacy/per-system keys still in use: `pjboy.voxelCharacter.v1`, `pjboy.voxelChar.v1`, `pjboy.voxelWeapons.owned.v1`, `pjboy.voxelFpTune.v1`, `pjboy.voxelTpTune.v1`, `pjboy.voxelInvTab.v1`, `pjboy.voxelHotbar.v1`.
